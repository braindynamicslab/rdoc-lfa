...
# columns_to_remove <- c("SSMA9434","SSMA9436")
# data0 <- data0[, !(names(data0) %in% columns_to_remove)]
...
# cfa.loadings[cfa.loadings < .4 & cfa.loadings > -.4] <- 0
...
Warning: lavaan WARNING:
    ...
    this check.Warning: lavaan WARNING: fit measures not available if model did not converge
```{r, fig.height = 8, fig.width = 8}
...
```
Remove SPPO4262 for cfanobi.fit.model
```{r, fig.height = 8, fig.width = 8}
...
```
Removed NVL982 and PVRR724 variance set to 0
```{r, fig.height = 8, fig.width = 8}
...
#change confidence interval of RMSEA
```{r, fig.height = 8, fig.width = 6}
library(MBESS)
#Change CI of RMSEA
m.cfa.fit.rmsea = fitmeasures(cfa.fit, c("rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust")) # Get RMSEA and its CI
m.cfa.fit.n = lavInspect(cfa.fit, what = "nobs") # Get N used in model
m.cfa.fit.dof = fitmeasures(cfa.fit, "df") # Get the dof in model
cfa.fit.ci.rmsea = ci.rmsea(m.cfa.fit.rmsea[1], m.cfa.fit.dof, m.cfa.fit.n, conf.level=.95) #Gives 95% CI for RMSEA

m.cfanobi.fit.rmsea = fitmeasures(cfanobi.fit, c("rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust")) # Get RMSEA and its CI
m.cfanobi.fit.n = lavInspect(cfanobi.fit, what = "nobs") # Get N used in model
m.cfanobi.fit.dof = fitmeasures(cfanobi.fit, "df") # Get the dof in model
cfanobi.fit.ci.rmsea = ci.rmsea(m.cfanobi.fit.rmsea[1], m.cfanobi.fit.dof, m.cfanobi.fit.n, conf.level=.95) #Gives 95% CI for RMSEA

m.rdoc.fit.rmsea = fitmeasures(rdoc.fit, c("rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust")) # Get RMSEA and its CI
m.rdoc.fit.n = lavInspect(rdoc.fit, what = "nobs") # Get N used in model
m.rdoc.fit.dof = fitmeasures(rdoc.fit, "df") # Get the dof in model
rdoc.fit.ci.rmsea = ci.rmsea(m.rdoc.fit.rmsea[1], m.rdoc.fit.dof, m.rdoc.fit.n, conf.level=.95) #Gives 95% CI for RMSEA

m.rdocbi.fit.rmsea = fitmeasures(rdocbi.fit, c("rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust")) # Get RMSEA and its CI
m.rdocbi.fit.n = lavInspect(rdocbi.fit, what = "nobs") # Get N used in model
m.rdocbi.fit.dof = fitmeasures(rdocbi.fit, "df") # Get the dof in model
rdocbi.fit.ci.rmsea = ci.rmsea(m.rdocbi.fit.rmsea[1], m.rdocbi.fit.dof, m.rdocbi.fit.n, conf.level=.95) #Gives 95% CI for RMSEA
```
...
```{r, fig.height = 8, fig.width = 6}
# load("cfa.fit.fullbrain42_5kbs.RData")
# load("rdoc.fit.fullbrain42_5kbs.RData")
# load("cfanobi.fit.fullbrain42_5kbs.RData")
# load("rdocbi.fit.fullbrain42_5kbs.RData")

load("cfa.fit.fullbrain42_5kbsyuan.RData")
load("rdoc.fit.fullbrain42_5kbsyuan.RData")
load("cfanobi.fit.fullbrain42_5kbsyuan.RData")
load("rdocbi.fit.fullbrain42_5kbsyuan.RData")
...
cfa.srmr.ci = quantile(cfa.fit.bs[, 6], probs = c(.025, .975), na.rm = TRUE)
...
rdoc.srmr.ci = quantile(rdoc.fit.bs[, 6], probs = c(.025, .975), na.rm = TRUE)
...
cfanobi.srmr.ci = quantile(cfanobi.fit.bs[, 6], probs = c(.025, .975), na.rm = TRUE)
...
rdocbi.srmr.ci = quantile(rdocbi.fit.bs[, 6], probs = c(.025, .975), na.rm = TRUE)
...
fitdata <- data.frame(Models = c("RDoC", "RDoC (bifactor)", "DD (bifactor)", "DD"), 
                   ...
                   errmin = c(rdoc.fit.ci.rmsea[[1]], rdocbi.fit.ci.rmsea[[1]], cfa.fit.ci.rmsea[[1]], cfanobi.fit.ci.rmsea[[1]]), 
                   errmax = c(rdoc.fit.ci.rmsea[[3]], rdocbi.fit.ci.rmsea[[3]], cfa.fit.ci.rmsea[[3]], cfanobi.fit.ci.rmsea[[3]]),
                   ...
                   srmr = c(rdoc.fit.srmr[1], rdocbi.fit.srmr[1], cfa.fit.srmr[1], cfanobi.fit.srmr[1]),
                   srmr.errmin = c(rdoc.srmr.ci[[1]], rdocbi.srmr.ci[[1]], cfa.srmr.ci[[1]], cfanobi.srmr.ci[[1]]),
                   srmr.errmax = c(rdoc.srmr.ci[[2]], rdocbi.srmr.ci[[2]], cfa.srmr.ci[[2]], cfanobi.srmr.ci[[2]])
                   )
                   
ggplot(fitdata, aes(x = Models, y = Robust.RMSEA)) +
  geom_bar(stat='identity', color="black", fill="skyblue", width=0.5) + 
  geom_errorbar(aes(x = Models, ymin=errmin, ymax=errmax), width=0.4, colour="orange", size=1.3) +
...
ggplot(fitdata, aes(x = Models, y = srmr)) +
  geom_bar(stat='identity', color="black", fill="skyblue", width=0.5) + 
  geom_errorbar(aes(x = Models, ymin=srmr.errmin, ymax=srmr.errmax), width=0.4, colour="orange", size=1.3) +
  scale_y_continuous(expand = expansion(mult = c(0, .1)))+ 
  ggtitle("srmr")+
  labs(y = "srmr")+
  theme_classic()
...
# fitdata_fullbrain5k[, (ncol(fitdata_fullbrain5k) - 5):ncol(fitdata_fullbrain5k)] <- round(fitdata_fullbrain5k[, (ncol(fitdata_fullbrain5k) - 5):ncol(fitdata_fullbrain5k)], 0)
...
#write.csv(data_nog, "loading_wholebrain.csv", row.names = TRUE)
```
...
for (var_name in colnames(data_ns)) {
  # Initialize a vector to store the sums of products for each factor
  sums_of_products <- numeric(length(colnames(dd.cbfa.fscores_nog)))
  ...
  for (i in 1:length(colnames(dd.cbfa.fscores_nog))) {
    ...
    sums_of_products[i] <- product_sum
  }
  
  # Find the factor with the highest sum of products
  best_factor_index <- which.max(sums_of_products)
  best_factor <- colnames(dd.cbfa.fscores_nog)[best_factor_index]
  
  # Create an assignment string without repeating the factor name
  assignment <- paste(var_name)
  ...
  assignments_list[[best_factor]] <- c(assignments_list[[best_factor]], assignment)
}

# Combine assignments into the final CFA model specification
cfa.testfit.model <- sapply(names(assignments_list), function(factor_name) {
  paste(factor_name, "=~", paste(assignments_list[[factor_name]], collapse = " + "))
})
...
for (var_name in colnames(data_ns)) {
  # Initialize a vector to store the sums of products for each factor
  sums_of_products <- numeric(length(colnames(rdoc.cfa.fscores)))
  ...
  # Find the factor with the highest sum of products
  best_factor_index <- which.max(sums_of_products)
  best_factor <- colnames(rdoc.cfa.fscores)[best_factor_index]
...
# Combine assignments into the final CFA model specification with adjusted coefficients
rdoc2.testfit.model <- sapply(names(assignments_list), function(factor_name) {
  factor_assignments <- assignments_list[[factor_name]]
  highest_sum <- highest_sums[[factor_name]]
...
g_definition <- paste("g =~", paste(all_variables, collapse = " + "))
rdoc2.testfit.model <- c(g_definition, rdoc2.testfit.model)

# Print the final CFA model specification
cat(paste(rdoc2.testfit.model, collapse = "\n"))
```
...
library(MBESS)
#Change CI of RMSEA
m.cfa.testfit.rmsea = fitmeasures(cfa.testfit, c("rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust")) # Get RMSEA and its CI
m.cfa.testfit.n = lavInspect(cfa.testfit, what = "nobs") # Get N used in model
m.cfa.testfit.dof = fitmeasures(cfa.testfit, "df") # Get the dof in model
cfa.testfit.ci.rmsea = ci.rmsea(m.cfa.testfit.rmsea[1], m.cfa.testfit.dof, m.cfa.testfit.n, conf.level=.95) #Gives 95% CI for RMSEA

m.rdoc.testfit.rmsea = fitmeasures(rdoc.testfit, c("rmsea.robust", "rmsea.ci.lower.robust", "rmsea.ci.upper.robust")) # Get RMSEA and its CI
m.rdoc.testfit.n = lavInspect(rdoc.testfit, what = "nobs") # Get N used in model
m.rdoc.testfit.dof = fitmeasures(rdoc.testfit, "df") # Get the dof in model
rdoc.testfit.ci.rmsea = ci.rmsea(m.rdoc.testfit.rmsea[1], m.rdoc.testfit.dof, m.rdoc.testfit.n, conf.level=.95) #Gives 95% CI for RMSEA
```
...
# Custom function to compute fit measures only for admissible solutions
computeFitMeasuresIfAdmissible <- function(model) {
  # Retrieve parameter estimates
  params <- parameterEstimates(model)
  
  # Check for nonadmissible solutions by looking for infinite or NaN estimates
  if(any(is.infinite(params$est) | is.nan(params$est))) {
    return(list(cf = NA, tli = NA, aic = NA, bic = NA, rmsea = NA, srmr = NA)) # Return NA for all fit measures for nonadmissible solutions
  } else {
    # Compute and return fit measures for admissible solutions
    return(fitMeasures(model, fit.measures = c("cfi.robust","tli.robust", "aic", "bic","rmsea.robust","srmr")))
  }
}

# Use the custom function in bootstrapLavaan calls
# Replace 'rdoc.testfit' and 'cfa.testfit' with your actual lavaan model objects

rdoc.testfit.bs <- bootstrapLavaan(rdoc.testfit, R = 5000, type = "yuan", FUN = computeFitMeasuresIfAdmissible)

cfa.testfit.bs <- bootstrapLavaan(cfa.testfit, R = 5000, type = "yuan", FUN = computeFitMeasuresIfAdmissible)

# save(rdoc.testfit.bs, file = "rdoc42.testfit.ns_5kbschtgpt.RData")
# save(cfa.testfit.bs, file = "cfa42.testfit.ns_5kbschtgpt.RData")
```

```{r, fig.height = 3, fig.width = 3}
load("rdoc42.testfit.ns_5kbs.RData")
load("cfa42.testfit.ns_5kbs.RData")
...
```{r, fig.height = 4, fig.width = 4}
plotDualHistograms <- function(data1, data2, col1 = rgb(1, 0, 0, 0.5), col2 = rgb(0, 0, 1, 0.5), label1 = "Dataset 1", label2 = "Dataset 2", breaks = 20) {
  # Clean data by removing NA, NaN, or infinite values
  data1_clean <- data1[!is.na(data1) & is.finite(data1)]
  data2_clean <- data2[!is.na(data2) & is.finite(data2)]
  
  # Determine xlim based on the range of both datasets
  combined_data <- c(data1_clean, data2_clean)
  xlim_range <- range(combined_data, na.rm = TRUE)
  
  # Plot the first histogram
  hist(data1_clean, col = col1, xlim = xlim_range, main = "Comparison of Histograms", xlab = "Value", ylab = "Frequency", breaks = breaks)
  
  # Plot the second histogram on top of the first
  hist(data2_clean, col = col2, add = TRUE, breaks = breaks)
  
  # Add a legend
  legend("topright", legend = c(label1, label2), fill = c(col1, col2))
}

# Assuming rdoc.testfit.bs[, 1] and cfa.testfit.bs[, 1] are your datasets
plotDualHistograms(rdoc.testfit.bs[, 1], cfa.testfit.bs[, 1], label1 = "RDOC Test Fit", label2 = "CFA Test Fit") # cfi.robust
plotDualHistograms(rdoc.testfit.bs[, 2], cfa.testfit.bs[, 2], label1 = "RDOC Test Fit", label2 = "CFA Test Fit") # tli.robust
plotDualHistograms(rdoc.testfit.bs[, 3], cfa.testfit.bs[, 3], label1 = "RDOC Test Fit", label2 = "CFA Test Fit") # aic
plotDualHistograms(rdoc.testfit.bs[, 4], cfa.testfit.bs[, 4], label1 = "RDOC Test Fit", label2 = "CFA Test Fit") # bic
plotDualHistograms(rdoc.testfit.bs[, 5], cfa.testfit.bs[, 5], label1 = "RDOC Test Fit", label2 = "CFA Test Fit") # rmsea.robust
...
fitdata <- data.frame(Models = c("RDoC", "DD"), 
                   ...
                   errmin = c(m.rdoc.testfit.rmsea[[2]], m.cfa.testfit.rmsea[[2]]), 
                   errmax = c(m.rdoc.testfit.rmsea[[3]], m.cfa.testfit.rmsea[[3]]),
...
ggplot(fitdata, aes(x = Models, y = Robust.RMSEA)) +
  geom_bar(stat='identity', color="black", fill="skyblue", width=0.5) + 
  geom_errorbar(aes(x = Models, ymin=errmin, ymax=errmax), width=0.4, colour="orange", linewidth=1.3) +
  scale_y_continuous(expand = expansion(mult = c(0, .1)))+ 
  ggtitle("Robust RMSEA")+
  labs(y = "RMSEA") +
  theme_classic()
...
```{r, fig.height = 4, fig.width = 4}
# num_missing <- sum(is.na(cfa.testfit.bs[, 3]))
# print(num_missing)
# num_missing <- sum(is.na(rdoc.testfit.bs[, 3]))
# print(num_missing)

for (i in 1:5) {
  # Omit missing values
  cfa_values <- cfa.testfit.bs[, i][!is.na(cfa.testfit.bs[, i])]
  rdoc_values <- rdoc.testfit.bs[, i][!is.na(rdoc.testfit.bs[, i])]
  differences <- cfa_values - rdoc_values
...
correlation_matrix <- matrix(NA, ncol(fscores1), ncol(fscores2))
rownames(correlation_matrix) <- colnames(fscores1)
colnames(correlation_matrix) <- colnames(fscores2)

# Loop through each column in fscores1 and compute correlations with columns in fscores2
for (i in 1:ncol(fscores1)) {
  for (j in 1:ncol(fscores2)) {
    correlation_matrix[i, j] <- cor(fscores1[, i], fscores2[, j])
  }
}

data = correlation_matrix
# desired_row_order <- c("SS", "PV", "SP", "CS", "NV")
# desired_col_order <- c("PA3", "PA1", "PA4", "PA2", "PA6", "PA5")
# Reorder rows and columns
# correlation_matrix <- data[match(desired_row_order, rownames(data)), match(desired_col_order, colnames(data))]

# Create a heatmap with correlation coefficients displayed
heatmap.2(data.matrix(correlation_matrix), dendrogram = "none", Rowv = NA, Colv = NA, trace = "none", 
           col = colorRampPalette(c("blue", "white", "red"))(100), 
           cellnote = round(correlation_matrix, 2), notecol = "black", notecex = 1.2)
...
```{r, fig.height = 5, fig.width = 6}
# (Using absolute values) Correlation between f-score coefficients of dd bifactor and rdoc spec model
fscores1 = abs(rdoc.cfa.fscores)
fscores2 = abs(dd.cbfa.fscores_nog)
