---
title: "Core model using whole-brain activation maps and External validation using coordinate activation maps"
author: "Shaun Quah"
date: "2023-02-07"
...
```{r, fig.height = 8, fig.width = 8}
data_ns <- import('Dataset Maps.xlsx',sheet='Coordinate Maps')
...
for (i in 1:5) {
  ...
  # Calculate the effect size (r) for Mann-Whitney U test
  n1 <- sum(!is.na(rdoc.testfit.bs[, i]))  # Sample size group 1
  n2 <- sum(!is.na(cfa.testfit.bs[, i]))  # Sample size group 2
  N <- n1 + n2  # Total observations

  # Approximate Z from the W-statistic
  z_score <- (w_statistic - (n1 * n2) / 2) / sqrt(n1 * n2 * (n1 + n2 + 1) / 12)
  effect_size_r <- z_score / sqrt(N)
